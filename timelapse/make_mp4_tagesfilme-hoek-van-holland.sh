...
timelapseDir="$baseDir/zeitraffer-hoek-van-holland"
timelapseDir="$baseDir/zeitraffer-hoek-van-holland"
outDir="$baseDir/zeitraffer-hoek-van-holland"

JpgDir=$baseDir"/zeitraffer-hoek-van-holland"
OutFile=$baseDir"/ffmpeg-timeleape-"$(date +"-%Y-%m-%d--%H-%M")".mp4"
Sound=$baseDir"/Sound/SoundCloud-Nature-of-Another.mp3"
...
#					/lager2/WebCams/find_blanks.sh
					echo " $mp4name existiert nicht -> erzeuge Film"
					CMD="ffmpeg -thread_queue_size 512 -framerate 30 -pattern_type glob -i \"$ordner/*.jpg\" -i $Sound -shortest  -c:v libx264 -acodec copy -flags global_header -pix_fmt yuv420p -s 800x600  $mp4name &"
					echo "-----------------------------------------"
					echo $CMD
					echo "-----------------------------------------"
					eval $CMD
